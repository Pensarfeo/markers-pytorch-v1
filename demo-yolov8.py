...
from model.yolov8.semantic import Model as YoloV8
...
modelName = "Yolov8-cocoTest-noise-brighness-colortwicking-counterExamples"
...
try:
  model = torch.load(getCheckpointName(modelName, 4))
...
print('epochs*(5000//batchSize//accumulate):', epochs*(5000//batchSize//accumulate))
scheduler = lr_scheduler.LinearLR(optimizer, start_factor=0.7, end_factor=1e-3, total_iters=epochs*(5000//batchSize//accumulate))
...
def lossFn(pred, target):
   ...
   ce = lossCE(torch.squeeze(pred), torch.squeeze(target))
   loss += ce
...
def train(dataloader, epoch):
    ...
    lossCe = 0
    lossDi = 0
    for i, (x, y, c) in enumerate(tqdm(dataloader)):
      # xn = x[0].numpy()
      # yn = y[0].numpy()
      # cv2.imshow('back', xn)
      # cv2.waitKey(0)
